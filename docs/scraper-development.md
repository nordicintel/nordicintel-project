...
Start with the [verified schema and mapping references](repositories.md#verified-schema-references). The same metadata-only profile applies to scraper output; it does not define the processed observation or file-tracking artifact format.
...
5. **Produce repeatable local outputs.** Generate separate metadata-only documents, processed observations, and traceable resource information. Metadata keeps `value: []` with complete dimensions and actual category counts. Document what triggers reprocessing and how reruns handle unchanged, revised, or newly discovered files. Keep incomplete processing distinguishable from valid output.
...
Emit a complete metadata document for each available supported language, without inventing translations. Preserve provider codes; if a source has no clear dataset code, resolve and document that mapping instead of assuming a filename or generated public ID satisfies the identity contract.
